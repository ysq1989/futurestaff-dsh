...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `FutureStaff-Agent-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'FutureStaff Agent.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing FutureStaff Agent.exe: ${portablePath}`)
  }
  ...
  assertPortableExecutableBuffer(
    ...
    `${portablePath}:FutureStaff Agent.exe`,
  )
  return portablePath
}
